...
3170 REM Calculate maximum steps before hitting quadrant boundary
3175 S9 = N
3180 IF ABS(X1) > .001 AND X1 > 0 THEN S9 = INT((8.9 - S1) / X1)
3185 IF ABS(X1) > .001 AND X1 < 0 THEN S9 = INT((S1 - 0.1) / (-X1))
3190 IF S9 > N THEN S9 = N
3195 T2 = S9
3196 IF ABS(X2) > .001 AND X2 > 0 THEN T2 = INT((8.9 - S2) / X2)
3197 IF T2 < S9 THEN S9 = T2
3200 T2 = S9
3201 IF ABS(X2) > .001 AND X2 < 0 THEN T2 = INT((S2 - 0.1) / (-X2))
3202 IF T2 < S9 THEN S9 = T2
3205 REM Move the ship S9 increments
3210 FOR I = 1 TO S9
3215 S1 = S1 + X1
3220 S2 = S2 + X2
3225 NEXT I
3230 REM If we didn't complete full movement, handle quadrant crossing
3235 IF S9 < N THEN GOTO 3500
